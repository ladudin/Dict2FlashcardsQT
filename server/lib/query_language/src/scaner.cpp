...
token::token() : type(token_type::NUL), lexeme(""), literal("") {}

token::token(token_type type) : type(type), lexeme(""), literal("") {}

token::token(token_type type, const std::string &lexeme)
    : type(type), lexeme(lexeme), literal("") {}
...
bool scanner::has_next(size_t i) { return current + i < source.size(); }

bool scanner::is_digit(char ch) { return std::isdigit(ch); }

void scanner::add_token(token_type type) { add_token(type, ""); }
...
void scanner::scan_token() {
  ...
  case '<':
    ...
    break;
  case '>':
    add_token(match(std::string("=")) ? tt::GREATER_EQUAL : tt::GREATER);
    break;
  case ' ':
  case '\r':
  case '\n':
