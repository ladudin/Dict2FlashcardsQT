...
/*
void interpreter::check_number_operands(token oper, const Value& left, const
Value& right) { std::visit([&](auto&& left_, auto&& right_) { using LeftType =
std::decay_t<decltype(left_)>; using RightType = std::decay_t<decltype(right_)>;

        if constexpr (std::is_same_v<LeftType, double> &&
std::is_same_v<RightType, double>) { if (right_ == 0 && oper.type == tt::SLASH)
