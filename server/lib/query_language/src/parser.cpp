...
bool parser::is_at_end() { return tokens[current].type == token_type::EOTF; }
...
std::unique_ptr<Expr> parser::unar() {
  if (match({tt::NOT, tt::MINUS, tt::LEN, tt::SPLIT, tt::UPPER, tt::LOWER,
             tt::REDUCE})) {
    token oper = previous();
...
std::unique_ptr<Expr> parser::multiplication() {
  ...
  while (match({tt::SLASH, tt::STAR})) {
    token oper = previous();
    std::unique_ptr<Expr> right = unar();
    left = std::make_unique<Binary>(std::move(left), oper, std::move(right));
  }
  return left;
}
...
std::unique_ptr<Expr> parser::and_expr() {
  ...
  while (match({tt::AND})) {
    ...
    left =
        std::make_unique<LogicalExpr>(std::move(left), oper, std::move(right));
  }
  return left;
}
...
std::unique_ptr<Expr> parser::expression() { return or_expr(); }

std::unique_ptr<Expr> parser::parse() { return expression(); }
